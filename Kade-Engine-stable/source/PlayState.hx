...
class PlayState extends MusicBeatState
{
	...
	public  static var isValidScore:Bool = true;
	...
	override public function create()
	{
		...
		switch (storyDifficulty)
		{
			...
			case 3:
				storyDifficultyText = "Bleeding Edge";
		}
			...
			case 'midna':
				{
					...
					var bg:FlxSprite = new FlxSprite(-100).loadGraphic(Paths.image('bg/sky', 'weekmidna'));
					bg.scrollFactor.set(0.1, 0.1);
					add(bg);

					var city:FlxSprite = new FlxSprite(-10).loadGraphic(Paths.image('bg/building', 'weekmidna'));
					city.scrollFactor.set(0.3, 0.3);
					city.setGraphicSize(Std.int(city.width * 0.85));
					city.updateHitbox();
					add(city);
					...
					for (i in 0...5)
					{
							var light:FlxSprite = new FlxSprite(city.x).loadGraphic(Paths.image('bg/building_lights', 'weekmidna'));
							light.scrollFactor.set(0.3, 0.3);
							light.visible = true;
							light.setGraphicSize(Std.int(light.width * 0.85));
							light.updateHitbox();
					...
					var street:FlxSprite = new FlxSprite(-40, 50).loadGraphic(Paths.image('bg/floor','weekmidna'));
					add(street);

					var streetBehind:FlxSprite = new FlxSprite(-40, 50).loadGraphic(Paths.image('bg/wall','weekmidna'));
					add(streetBehind);
				}
			default:
			{
		...
		gradient = new FlxSprite(0, 0).makeGraphic(FlxG.width, FlxG.height, FlxColor.BLACK);
		...
		kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,"KE CUSTOM FUNKIN' FOREVER BUILD", 16);
		...
		missLimitText.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		...
		if (isStoryMode)
		{
			trace("about to call school intro...");
			switch (SONG.song.toLowerCase().replace(' ', '-'))
			{
				...
				case 'thorns':
					schoolIntro(doof);
					trace("whoah the school intro is called");
				default:
					startCountdown();
			}
		}
		...
		new FlxTimer().start(0.3, function(tmr:FlxTimer)
		{
			...
			else
			{
				if (dialogueBox != null)
				{
					...
					{
						add(dialogueBox);
						trace("whoah the box should appear");
					}
				}
				else
					startCountdown();
				trace("whoah the game starts");
	...
	function startCountdown():Void
	{
		...
		if (executeModchart)
		{
			...
			luaModchart.executeState('start', [StringTools.replace(PlayState.SONG.song.toLowerCase(), " ", '-')]);
		}
		#end
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			var splash:FlxSprite = new FlxSprite(0, strumLine.y);
	...
	override public function update(elapsed:Float)
	{
		...
		if (executeModchart && luaModchart != null && songStarted)
		{
			...
			if (luaModchart.getVar("showOnlyStrums",'bool'))
			{
				...
				missLimitText.visible = false;
				missRating.visible = false;
	...
	function showSploosh(daNote:Note, daRating:String)
        {
			...
			for (i in 0...4)
			{
				sploosh.animation.addByPrefix('splash 0 ' + i, 'note impact 1 ' + colorName[i], 24, false);
				sploosh.animation.addByPrefix('splash 1 ' + i, 'note impact 2 ' + colorName[i], 24, false);
			}
